...
| CPU Mining Framework | Institutional Framework | Provides coin-agnostic CPU mining-session composition, IRIA-granted thread placement, normalized performance intelligence, advisory optimization, safety reporting, and CPU Performance Digital Twin records. Implemented as ICMF (`core/cpu_mining_framework/`, PHASE-25). |
| GPU Mining Framework | Institutional Framework | Provides coin-agnostic UUID-scoped multi-GPU mining-session composition, IRIA-governed VRAM/compute-queue integration, normalized performance intelligence, advisory optimization, failure reporting, and GPU Mining Digital Twin records. Implemented as IGMF (`core/gpu_mining_framework/`, PHASE-26). |
| ASIC Mining Framework | Institutional Framework | Provides vendor-neutral UUID-scoped multi-ASIC mining-session composition, IRIA-governed device/hashboard integration, normalized performance intelligence, advisory optimization, failure reporting, and ASIC Fleet Digital Twin records. Implemented as IAMF (`core/asic_mining_framework/`, PHASE-27). |
| Miner Process Manager | Institutional Authority | Safely manages the lifecycle, security gates, resource-governed launch, supervision, restart, and termination of external miner processes. Implemented as IMPM (`core/miner_process_manager/`, PHASE-28). |
| Mining Statistics Framework | Institutional Framework | Provides the institutional canonical mining-telemetry layer: data quality, deterministic anomaly detection, multi-level aggregation, historical rollups, explainability, and Hardware-to-Statistics traceability. Implemented as IMSF (`core/mining_statistics_framework/`, PHASE-29). |
| Mining Plugin Runtime | Institutional Framework | Mechanical execution host for explicitly handed-in plugin instances; not an inventory or authorization authority. Implemented as IMPR (`core/mining_plugin_runtime/`, PHASE-30). |
| Mining Plugin Certification Authority | Institutional Authority | Establishes plugin/software trust and evidence-based certification, distinct from IHCA hardware certification. Implemented as IMPCA (`core/mining_plugin_certification_authority/`, PHASE-31). |
